...
/*=========================
  ...
  creates, binds a client side socket
...
/*=========================
  client_connect
  args: char * other_client

  other_client is a string representing the other client's address

  create and connect a socket to a client socket that is
...
int client_connect(char * other_client) {
    ...
    hints->ai_socktype = SOCK_STREAM;  //TCP socket
    getaddrinfo(other_client, PORT, hints, &results);
